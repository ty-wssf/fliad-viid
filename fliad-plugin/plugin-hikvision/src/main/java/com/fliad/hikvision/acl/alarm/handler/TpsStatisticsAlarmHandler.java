...
public class TpsStatisticsAlarmHandler implements AlarmHandler {
    ...
    public void handle(int lCommand, HCNetSDK.NET_DVR_ALARMER pAlarmer, Pointer pAlarmInfo, int dwBufLen, Pointer pUser) {
        ...
        String startTime = "" + String.format("%04d", netDvrTpsStatisticsInfo.struTPSStatisticsInfo.struStartTime.wYear) +
                ...
                String.format("%03d", netDvrTpsStatisticsInfo.struTPSStatisticsInfo.struStartTime.wMilliSec);
        ...
        for (int i = 0; i < totalLaneNum && i < netDvrTpsStatisticsInfo.struTPSStatisticsInfo.struLaneParam.length; i++) {
            ...
            // 设置时间占有率和空间占有率
            trafficFlowInfo.setSjzyl(laneParam.fTimeOccupyRation);
            trafficFlowInfo.setKjzyl(laneParam.fSpaceOccupyRation);
            ...
            options.addEncoder(Date.class, (data, node) -> {
                node.val().setString(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(data));
            });
            log.info("车道统计数据：{}", ONode.stringify(trafficFlowInfo, options));
            List<ResourceWorkflow> workflowList = alarmManager.getViidWorkflowService().findBySubscribeDetail("200");

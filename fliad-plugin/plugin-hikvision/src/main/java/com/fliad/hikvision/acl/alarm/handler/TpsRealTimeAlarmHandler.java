    ...
    public void handle(int lCommand, HCNetSDK.NET_DVR_ALARMER pAlarmer, Pointer pAlarmInfo, int dwBufLen, Pointer pUser) {
        ...
        try {
            ...
            for (ResourceWorkflow workflow : workflowList) {
                ...
                inputs.put("inputs", ONode.stringify(tpsParamDetail, options));

                Map<String, Object> inputs_ = new HashMap<>();
                inputs_.put("lCommand", lCommand);
                inputs_.put("pAlarmer", pAlarmer);
                inputs_.put("pAlarmInfo", pAlarmInfo);
                inputs_.put("dwBufLen", dwBufLen);
                inputs_.put("pUser", pUser);

                inputs.put("inputs_", inputs_);
                taskRunInput.setInputs(inputs);

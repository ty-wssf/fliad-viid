...
public class XptWordTableRenderer implements ITextTemplateOutput {
    ...
    protected void renderExpandedSheet(Writer out, ExpandedSheet sheet, IXptRuntime xptRt) throws IOException {
        ...
        for (int i = 0, n = table.getColCount(); i < n; i++) {
            ...
            Map<String, ValueWithLocation> gridColAttrs = new HashMap<>();
            gridColAttrs.put("w:w", vl(null, width));
            handler.simpleNode(null, "w:gridCol", gridColAttrs);
        }
    ...
    protected void renderProxyCell(ExpandedCell cell, IXNodeHandler handler) {
        ...
        if (realCell.getMergeAcross() > 0) {
            Map<String, ValueWithLocation> gridSpanAttrs = new HashMap<>();
            gridSpanAttrs.put("w:val", ValueWithLocation.of(null, realCell.getMergeAcross() + 1));
            handler.simpleNode(null, "w:gridSpan", gridSpanAttrs);
        }
        ...
        Map<String, ValueWithLocation> attrs = new HashMap<>();
        attrs.put("w:w", ValueWithLocation.of(null, width));
        attrs.put("w:type", ValueWithLocation.of(null, "dxa"));
    ...
    protected void renderCell(ExpandedCell cell, IXNodeHandler handler, IXptRuntime xptRt) {
        ...
        if (cell.getMergeAcross() > 0) {
            Map<String, ValueWithLocation> gridSpanAttrs = new HashMap<>();
            gridSpanAttrs.put("w:val", ValueWithLocation.of(null, cell.getMergeAcross() + 1));
            handler.simpleNode(null, "w:gridSpan", gridSpanAttrs);
        }

        if (cell.getMergeDown() > 0) {
            Map<String, ValueWithLocation> vMergeAttrs = new HashMap<>();
            vMergeAttrs.put("w:val", ValueWithLocation.of(null, "restart"));
            handler.simpleNode(null, "w:vMerge", vMergeAttrs);
        }

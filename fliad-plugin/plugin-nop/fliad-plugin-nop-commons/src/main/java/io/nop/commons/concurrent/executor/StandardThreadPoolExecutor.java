 ...
 */
class StandardThreadPoolExecutor extends ThreadPoolExecutor {
    /**
     * The number of tasks submitted but not yet finished. This includes tasks in the queue and tasks that have been
     * handed to a worker thread but the latter did not start executing the io.nop.task yet. This number is always
     ...
     */
    private long threadRenewalDelay = CommonConstants.DEFAULT_THREAD_RENEWAL_DELAY;
    ...
    private void initQueue(BlockingQueue<Runnable> workQueue) {
        if (workQueue instanceof TaskQueue) {
            ((TaskQueue) workQueue).setParent(this);
        }
    }
        ...
        if (currentThreadShouldBeStopped()) {
            ...
            if (lastTime + threadRenewalDelay < System.currentTimeMillis()) {
                if (lastTimeThreadKilledItself.compareAndSet(lastTime, System.currentTimeMillis() + 1)) {
                    ...
                    throw StopPooledThreadException.INSTANCE;
                }
            }
        }
    }
    ...
    protected boolean currentThreadShouldBeStopped() {
        if (threadRenewalDelay >= 0 && Thread.currentThread() instanceof NopThread) {
            NopThread currentTaskThread = (NopThread) Thread.currentThread();
     ...
     *
     * @param command the runnable io.nop.task
     * @param timeout A timeout for the completion of the io.nop.task
     * @param unit    The timeout time unit
     * @throws RejectedExecutionException if this io.nop.task cannot be accepted for execution - the queue is full

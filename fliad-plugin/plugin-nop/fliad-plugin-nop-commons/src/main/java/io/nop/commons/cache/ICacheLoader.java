...
public interface ICacheLoader<K, V> {
     ...
     * @throws InterruptedException if this method is interrupted. {@code InterruptedException} is treated like any other
     *                              {@code Exception} in all respects except that, when it is caught, the thread's interrupt status is
     *                              set
     */
    @Nullable
     ...
     * {@code keys} then all returned entries will be cached, but only the entries for {@code keys} will be returned
     * from {@code getAll}.
     * <p>
     ...
     * @return a future containing the map from each key in {@code keys} to the value associated with that key; <b>may
     * not contain null values</b>
     */
    @NonNull
    default CompletableFuture<Map<@NonNull K, @NonNull V>> asyncLoadAll(@NonNull Iterable<? extends K> keys,
     ...
     * @return the new value associated with {@code key}, or {@code null} if the mapping is to be removed
     * @throws Exception            or Error, in which case the mapping is unchanged
     * @throws InterruptedException if this method is interrupted. {@code InterruptedException} is treated like any other
     *                              {@code Exception} in all respects except that, when it is caught, the thread's interrupt status is
     *                              set
     ...
     * @return a future containing the new value associated with {@code key}, or containing {@code null} if the mapping
     * is to be removed
     */
    @NonNull

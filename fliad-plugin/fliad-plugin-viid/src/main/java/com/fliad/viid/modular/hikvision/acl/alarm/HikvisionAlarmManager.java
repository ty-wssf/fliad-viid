...
public class HikvisionAlarmManager {
     ...
     * @return 是否添加成功
     */
    public boolean addDevice(String deviceId, String deviceNumber, String ip, int port, String username, String password) {
        ...
        try {
            HikvisionDevice device = new HikvisionDevice(deviceId, deviceNumber, ip, port, username, password);
            devices.put(deviceId, device);
            log.info("成功添加设备: {} (设备编号: {})", deviceId, deviceNumber);
            return true;
        } catch (Exception e) {
     ...
     */
    public HikvisionDevice getDeviceByIp(String ip) {
        for (HikvisionDevice device : devices.values()) {

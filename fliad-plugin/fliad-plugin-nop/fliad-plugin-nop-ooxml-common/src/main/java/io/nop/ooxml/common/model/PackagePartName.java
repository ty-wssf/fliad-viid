...
import io.nop.ooxml.common.exceptions.InvalidFormatException;
...
public final class PackagePartName implements Comparable<PackagePartName> {
     ...
     * @param checkConformance Flag to specify if the constructor have to validate the OPC conformance. Must be always
     *                         <code>true</code> except for special URI like '/' which is needed for internal use by OpenXML4J but is
     *                         not valid.
     * @throws InvalidFormatException Throw if the specified part name is not conform to Open Packaging Convention specifications.
     * @see URI
     */
     ...
     * @return <code>true</code> if this part name respect the relationship part naming convention else
     * <code>false</code>.
     */
    ...
    /**
     * Throws an exception (of any kind) if the specified part name does not follow the Open Packaging Convention
     * specifications naming rules.
     *
     * @param partUri The part name to check.
     * @throws InvalidFormatException Throws if the part name is invalid.
     */
     ...
     * <p>
     * Throws an exception if a segment contain percent-encoded forward slash ('/'), or backward slash ('\') characters.
     * [M1.7]
     * <p>
     * Throws an exception if a segment contain percent-encoded unreserved characters. [M1.8]
     * <p>
     * Throws an exception if the specified part name's segments end with a dot ('.') character. [M1.9]
     * <p>
     * Throws an exception if a segment doesn't include at least one non-dot character. [M1.10]
     *
     * @param partUri The part name to check.
     * @throws InvalidFormatException if the specified URI contain an empty segments or if one the segments contained in the part name,
    ...
    private static void throwExceptionIfPartNameHaveInvalidSegments(URI partUri) throws InvalidFormatException {
        ...
        String[] segments = partUri.toASCIIString().replaceFirst("^" + PackagingURIHelper.FORWARD_SLASH_CHAR, "")
     ...
     *
     * @param partUri The part name to check.
     * @throws InvalidFormatException If the specified part name doesn't start with a forward slash character '/'.
     */
    private static void throwExceptionIfPartNameNotStartsWithForwardSlashChar(URI partUri)
        ...
        if (uriPath.length() > 0 && uriPath.charAt(0) != PackagingURIHelper.FORWARD_SLASH_CHAR) {
            throw new InvalidFormatException(
                    "A part name shall start with a forward slash ('/') character [M1.4]: " + partUri.getPath());
        }
    }

    /**
     * Throws an exception if the specified part name ends with a forwar slash character '/'. [M1.5]
     *
     * @param partUri The part name to check.
    ...
    private static void throwExceptionIfPartNameEndsWithForwardSlashChar(URI partUri) throws InvalidFormatException {
        ...
        if (uriPath.length() > 0 && uriPath.charAt(uriPath.length() - 1) == PackagingURIHelper.FORWARD_SLASH_CHAR) {
            throw new InvalidFormatException(
                    "A part name shall not have a forward slash as the last character [M1.5]: " + partUri.getPath());
        }
    }
     ...
     * <p>
     * Part name equivalence is determined by comparing part names as case-insensitive ASCII strings. Packages shall not
     * contain equivalent part names and package implementers shall neither create nor recognize packages with
     * equivalent part names. [M1.12]
     */
    @Override
    ...
    /**
     * Retrieves the extension of the part name if any. If there is no extension returns an empty String. Example :
     * '/document/content.xml' =&gt; 'xml'
     *
    ...
    /**
     * A natural sort order for package part names, consistent with the requirements of {@code java.util.Comparator},
     * but simply implemented as a static method.
     * <p>
     * For example, this sorts "file10.png" after "file2.png" (comparing the numerical portion), but sorts "File10.png"
     * before "file2.png" (lexigraphical sort)
     *
     * <p>
    ...
    /**
     * A natural sort order for strings, consistent with the requirements of {@code java.util.Comparator}, but simply

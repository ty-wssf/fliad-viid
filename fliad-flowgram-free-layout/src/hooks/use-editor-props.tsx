    ...
    () => ({
      ...
      onContentChange: debounce((ctx, event) => {
        if (ctx.document.disposed) return;
          ...
          console.log('Auto Save: ', event, jsonData);
        ...
        console.log('--- Playground init ---');
          ...
          if (flowId) {
              fetch(`/viid/workflow/detail?id=${flowId}`, {
                  ...
                  .then(data => {
                      // 使用接口数据初始化画布
                      if (JSON.parse(data.data.content).nodes.length !== 0) {
                          ctx.document.clear()
                          ctx.document.fromJSON(JSON.parse(data.data.content));
                          // ctx.document.fitView(false);
                          console.log('画布数据初始化成功');
                          ctx.tools.autoLayout();
                          ctx.tools.fitView(true);
                      }
                  })
                  .catch(error => {
                      console.error('初始化画布数据失败:', error);
                  });
          } else {
              console.warn('无法初始化：URL中未找到流程ID');
          }
      },
      /**
       * Playground render

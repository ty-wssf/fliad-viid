 ...
 * 用于管理特定类型设备的在线/离线状态
 *
 * 注意：推荐各业务模块创建自己的 DeviceStateManager 实例来管理设备状态，
 * 然后通过该实例获取特定设备类型的 DeviceTypeStateManager 来进行操作，
 * 以避免设备类型标识符冲突问题。
 *
 * @author lingma
 * @date 2025/10/30
 */
public class DeviceTypeStateManager {
    
    private static final Logger log = LoggerFactory.getLogger(DeviceTypeStateManager.class);
     ...
     * 定时检测功能开关
     */
    private final java.util.concurrent.atomic.AtomicBoolean enableScheduledCheck = new java.util.concurrent.atomic.AtomicBoolean(false);
    ...
    private final String onlineEntityCachePrefix;
    
    public DeviceTypeStateManager(CommonCacheOperator cacheOperator, String onlineEntityCachePrefix, String deviceType) {
        this.cacheOperator = cacheOperator;
        this.onlineEntityCachePrefix = onlineEntityCachePrefix;
     ...
     */
    public boolean handleOnlineEvent(String entityId) {
        ...
        // 更新缓存
        cacheOperator.put(onlineEntityCachePrefix + entityId, "1");
     ...
     * @return true-状态已变更，false-状态未变更
     */
    public boolean handleOfflineEvent(String entityId) {
        if (StrUtil.isBlank(entityId)) {
        ...
        // 清除缓存
        cacheOperator.remove(onlineEntityCachePrefix + entityId);
    ...
    public int performScheduledCheck() {
        ...
        for (String entityId : getOnlineEntities()) {
            ...
            if (lastActiveTime != null && (currentTime - lastActiveTime) > entityTimeoutMillis) {
                // 实体超时，标记为离线
                if (handleOfflineEvent(entityId)) {
                    offlineCount++;
                }
            }
        }

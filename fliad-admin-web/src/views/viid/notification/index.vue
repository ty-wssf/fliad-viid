	...
	<a-card :bordered="false">
		...
		>
			<template #operator class="table-operator">
				<a-space>
					<a-button type="primary" @click="formRef.onOpen()" v-if="hasPerm('viidNotificationAdd')">
						<template #icon><plus-outlined /></template>
						新增
					</a-button>
					<xn-batch-button
						v-if="hasPerm('viidNotificationBatchDelete')"
						buttonName="批量删除"
						icon="DeleteOutlined"
			...
			<template #bodyCell="{ column, record }">
				<template v-if="column.dataIndex === 'action'">
					...
					<a-divider type="vertical" v-if="hasPerm(['viidNotificationEdit', 'viidNotificationDelete'], 'and')" />
					<a @click="formRef.onOpen(record)" v-if="hasPerm('viidNotificationEdit')">编辑</a>
					<a-divider type="vertical" v-if="hasPerm(['viidNotificationEdit', 'viidNotificationDelete'], 'and')" />
					<a-popconfirm title="确定要删除吗？" @confirm="deleteViidSubscribeNotifications(record)">
						<a-button type="link" danger size="small" v-if="hasPerm('viidNotificationDelete')">删除</a-button>
					</a-popconfirm>
				</template>
			</template>
		</s-table>
...
<script setup name="viidNotification">
	...
	// 操作栏通过权限判断是否显示
	if (hasPerm(['viidNotificationDetail', 'viidNotificationEdit', 'viidNotificationDelete'])) {
		columns.push({
			title: '操作',
			dataIndex: 'action',
			align: 'center',
